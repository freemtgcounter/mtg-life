...
class Header extends Component {
  ...
  new40LifeButtonClick = () => {
    if (window.confirm('Really start a new game?')) {
      ...
      this.props.reduxState.players.players.forEach(p => {
        this.props.reduxState.players.players.forEach(op => {
          if (p.id !== op.id) {
            return (this.props.dispatch(addEdh({pid: p.id, oid: op.id})))
          }
        })
        ...
        <header className={this.props.reduxState.game.settingsOpen ? 'open' : 'close'}>
          <div>
            ...
            <section id="game-settings">
              <div>
                <label for="player-add">Add a <span className="new">New</span> Player</label>
                <input id="player-add" ref={(el) => this.nameFieldRef = el} type="text" onChange={this.handleChange} />
                <button onClick={this.handlePlayerAdd}>Add</button>
              </div>
              <div className="edh-damange">
